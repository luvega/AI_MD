...
本章先读取根目录 `大纲.md`，再读取 `chapters/chapter-07/本章大纲.md`。正文使用第 05 章原始学习素材的全文提取页、OCR 清洗摘要、章节精读、方法笔记和 claims 矩阵。正文不复制原始 PDF、课件截图、Office 文件或压缩包，也不把已删除的旧发布层作为正文来源。

| 材料 | 本章使用方式 | 边界 |
|:---|:---|:---|
| `大纲.md` | 确认第 7 章在全书中位于轨迹分析之后、AI 亲和力预测之前 | 不把第 8 章 Boltz2 和 AI affinity 内容提前展开 |
| `chapters/chapter-07/本章大纲.md` | 确认本章问题、小节结构、证据边界和待确认项 | 待确认项不扩写成正式事实 |
| `06_原始学习素材/第05章_亲和力计算/全文提取/.../pages/` | 提取结合自由能、方法谱系、`gmx_MMPBSA` 输入、命令、参数和结果解释 | OCR 或课件截图中的数值不写成本项目结果 |
| `OCR清洗摘要.md` | 判断哪些页可用于检索、哪些页只适合人工复核 | 低质量 OCR 不作为正文事实来源 |
| `01_课程章节索引/章节精读/第05章_AI多组分亲和力计算精读.md` | 补充本章与全书亲和力计算主题的关系 | 不替代逐页素材核对 |
| `02_方法笔记/亲和力模型综述.md` | 统一 docking、MM/PBSA、FEP 和 AI 模型的任务差异 | 不把不同方法输出强行换算 |
| `07_研究工作台/证据与claims矩阵.md` | 约束 claim 级别，区分材料、方法输出、允许解释和后续验证 | 不用单一计算结果证明机制或药效 |
...
本章待作者确认的项目包括：`gmx_MMPBSA` 官方文档与引用信息、可公开教学数据包、真实或教学输出文件、Uni-GBSA 的正式来源与命令示例，以及全书目录中 `gmx_ana` 是否统一写作 `gmx_MMPBSA_ana`。这些内容补齐后，可以继续生成配套脚本、示例参数文件和结果解析模板。
