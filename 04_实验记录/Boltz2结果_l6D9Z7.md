...
| 指标 | 数值 | 解释 |
|:---|---:|:---|
| `confidence_scores` | 0.909981 | 整体置信度较高。 |
| `ptm_scores` | 0.922580 | 结构拓扑预测可信度较高。 |
| `iptm_scores` | 0.959715 | 界面预测可信度较高。 |
| `complex_plddt_scores` | 0.897548 | 复合物局部置信度较高。 |
| `complex_iplddt_scores` | 0.864698 | 界面局部置信度较高。 |
| `affinity_LIG_affinity_probability_binary` | 0.921875 | 结合倾向预测概率较高。 |
| `affinity_LIG_affinity_pic50` | 8.345176 | 亲和力读数，可用于候选排序。 |
| `l6D9Z7_model_0_protein_mean_plddt` | 90.196774 | 蛋白平均 pLDDT 高。 |
| `l6D9Z7_model_0_ligand_mean_plddt` | 87.28 | 配体局部置信度高。 |

## 当前判断

该结果适合作为第五章 Boltz2 解析示例：结构置信度、界面置信度和亲和力读数都较完整。后续如果用于报告，应补充靶点、配体身份、输入文件来源和可视化截图。
