...
`jumper_highly_2021` 是 AlphaFold2 的核心结构预测文献。它支撑本章关于“预测结构可以作为结构假设和大规模建模入口”的背景，但也提醒读者：预测结构需要结合置信度、区域功能和下游任务解释。AlphaFold2 对单体结构的推进很大，但不能自动解决复合物、配体结合、构象动态和实验功能问题。

`abramson_accurate_2024` 是 AlphaFold3 复合物预测的重要文献。它把结构预测从单体蛋白拓展到蛋白、核酸、小分子、离子和修饰等多类分子互作，为后续多组分建模提供了重要背景。本章使用它时强调边界：AlphaFold3 的复合物预测可以帮助生成互作假设和结构图，但仍不能替代结合实验、动力学解释和药效验证。

`akdel_structural_2022` 是 AlphaFold2 应用的结构生物学社区评估。它适合支撑课程中的审慎态度：预测结构在很多任务中有价值，但不同任务对精度、构象状态和局部区域的要求不同。对药物设计来说，活性口袋、构象变化、配体诱导适配和界面细节往往比整体 fold 更关键。

完整引用见 [附录 C](../appendices/references.md)。本章不把 PyMOL 或 Chimera 操作本身作为正式文献锚点；具体命令以本地方法卡和软件官方文档为主，课程引用以结构预测和结构应用文献为主。
